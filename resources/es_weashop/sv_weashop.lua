...
MySQL:open("127.0.0.1", "gta5_script_weaponshop", "root", "")
...
AddEventHandler('CheckMoneyForWea', function(weapon,price)
	TriggerEvent('es:getPlayerFromId', source, function(user)
		...
		if (tonumber(user.money) >= tonumber(price)) then
			...
			local nb_weapon = 0
			local executed_query = MySQL:executeQuery("SELECT * FROM owned WHERE identifier = '@name'",{['@name'] = player})
			...
			if (tonumber(max_number_weapons) > tonumber(nb_weapon)) then
				...
				MySQL:executeQuery("INSERT INTO owned (identifier,weapon,withdraw_cost) VALUES ('@username','@weapon','@cost')",
			...
			TriggerClientEvent("es_freeroam:notify", source, "CHAR_MP_ROBERTO", 1, "Roberto", false, "T'as pas assez d'argent !\n")
		end
	end)
end)
...
AddEventHandler("weaponshop:GiveWeaponsToPlayer", function(player)
	TriggerEvent('es:getPlayerFromId', player, function(user)
		...
		local executed_query = MySQL:executeQuery("SELECT * FROM owned WHERE identifier = '@name'",{['@name'] = playerID})
		local result = MySQL:getResults(executed_query, {'weapon','withdraw_cost'}, "identifier")
		...
		if(result)then
			...
			TriggerClientEvent("es_freeroam:notify", source, "CHAR_MP_ROBERTO", 1, "Roberto", false, "Hey, j'ai des armes pour toi !\n")
		end

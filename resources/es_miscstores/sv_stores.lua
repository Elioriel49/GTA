		...
		for k,v in pairs(shops)do
			if(get3DDistance(v.position.x, v.position.y, v.position.z, user.coords.x, user.coords.y, user.coords.z) < 3.0)then
				...
				if(price <= user.money)then
						...
						TriggerClientEvent('chatMessage', source, "SHOP", {255, 0, 0}, "Tu n'as pas assez d'argent, va travailler!.")
					end
				else
					TriggerClientEvent('chatMessage', source, "SHOP", {255, 0, 0}, "Tu ne peux pas acheter cette arme ici.")
				end

				return
			end
		end
		TriggerClientEvent('chatMessage', source, "SHOP", {255, 0, 0}, "Tu n'es pas dans un magasin.")
	end)
end)
	...
	TriggerEvent('es:getPlayerFromId', source, function(user)
		...
		if(get3DDistance(pos.x, pos.y, pos.z, lpos.x, lpos.y, lpos.z) < 1.0)then
			if(user.money > 250)then
				...
				TriggerClientEvent('chatMessage', source, 'SHOP', {255, 0, 0}, "Tu as récupéré toute ta vie.")
